...
def draw_loomis_overlay(img, mesh_points, faces, pitch, yaw, roll, tx, ty, scale=100, 
    ...
    COLORS = {
        'front': (255, 120, 0), 'back': (180, 100, 50),
    ...
    overlay, mask = np.zeros_like(img), np.zeros(img.shape[:2], dtype=np.uint8)
    ...
    if len(mesh_points) > 0:
        ...
        # Render depth sorted faces for the background mesh
    ...
    def draw_3d_segment(target, p1, p2, color, vibrant):
        ...
        if vibrant == is_front:
            ...
            cv2.line(target, pt1, pt2, color, 2)
    ...
    out = img.copy()
    indices = mask > 0
    # Combine original image with the muted background guides
    out[indices] = cv2.addWeighted(img[indices], 0.8, overlay[indices], 0.2, 0)

    # Render foreground guides for circles and side crosses
    for ring in rings:
        for i in range(len(ring)-1):
            if ring is rings[1] or abs(ring[i][0]) <= SIDE_DIST:
                draw_3d_segment(out, ring[i], ring[i+1], COLORS['front'], True)
    ...
    for side_x in [-SIDE_DIST, SIDE_DIST]:
        ...
        cross = [(np.array([side_x, -SIDE_RADIUS, 0]), np.array([side_x, SIDE_RADIUS, 0])),
                 (np.array([side_x, 0, -SIDE_RADIUS]), np.array([side_x, 0, SIDE_RADIUS]))]
        for i in range(len(side_pts)-1):
            draw_3d_segment(out, side_pts[i], side_pts[i+1], COLORS['front'], True)
        for p1, p2 in cross:
            draw_3d_segment(out, p1, p2, COLORS['front'], True)
    ...
    for i, pts in enumerate([jaw_pts_l, jaw_pts_r]):
        if pts:
            ...
            cv2.polylines(out, [np.array(full_jaw, np.int32)], False, COLORS['yellow'], 2, cv2.LINE_AA)

    # Add vibrant outline to the face mesh
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    cv2.drawContours(out, contours, -1, COLORS['front'], 2)

    # Calculate center of the eye line
    pt_eye_2d = get_2d(np.array([0, 0, 1.0]))
    if chin_2d: cv2.line(out, pt_eye_2d, chin_2d, COLORS['green'], 2)
        
    def draw_tick(center, length_factor, color):
        if center is None: return
        # Adjust tick length based on the current scale of the head
        actual_length = scale * length_factor
        half = (actual_length / scale) / 2
        r1, r2 = R @ np.array([-half, 0, 0]), R @ np.array( [half, 0, 0])
        cv2.line(out, (int(center[0] + r1[0] * scale), int(center[1] + r1[1] * scale)),
                      (int(center[0] + r2[0] * scale), int(center[1] + r2[1] * scale)), color, 5)

    # Drawing red ticks with dynamic length relative to scale
    tick_size = 0.25 # This represents 25 percent of the head radius
